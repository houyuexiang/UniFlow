# UniFlow Windows Install Script (with Auto-Start)
# Run PowerShell as Administrator, then execute: .\install-enable.ps1
# Installs UniFlow as a Windows service (LocalSystem account) and enables auto-start on boot.
...
sc.exe config $SERVICE_NAME start= auto obj= LocalSystem

Write-Info "Enabling auto-start on boot..."
...
Write-Host " UniFlow Installation Complete (Auto-Start Enabled)" -ForegroundColor Cyan
Write-Host "========================================" -ForegroundColor Cyan
